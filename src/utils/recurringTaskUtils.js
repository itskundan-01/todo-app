/**
 * Generate individual task instances from a recurring task pattern
 */
export const generateTaskInstances = (recurringTask) => {
  const {
    title,
    priority,
    recurrenceType,
  ...
  // Each recurring task gets a unique identifier that's used in all its instances
  const recurringId = _id;
  ...
  // Create task instances with consistent format
  return dates.map((date, index) => ({
    _id: `${recurringId}-${index}`,
    recurringId,
    text: title,
    priority,
    ...
    originalRecurrence: {
      type: recurrenceType,
      pattern: getPatternDescription(recurringTask)
    }
  }));
};
...
const generateDailyDates = (startDate, endDate, dailyDays) => {
  ...
  const daysToUse = dailyDays.length > 0 ? dailyDays : 
...
export const getPatternDescription = (task) => {
  ...
  switch (recurrenceType) {
    ...
    case 'daily':
      return dailyDays.length === 7 ? 'Every day' : `Every ${dailyDays.join(', ')}`;
    case 'weekly':
...
// Group task instances by date relative to today
export const groupTasksByDate = (taskInstances) => {
